...
function ESP:updateComponents(components, character, player)
    ...
    if hrp and humanoid then
        ...
        if onScreen then
            ...
            -- Atualizar rótulo de item
            if character:FindFirstChild("Item") then
                components.ItemLabel.Visible = self.toggles.ItemLabel
                components.ItemLabel.Text = character.Item.Value -- Adapte conforme necessário
                components.ItemLabel.Position = Vector2.new(hrpPosition.X, hrpPosition.Y - height / 2 - 30)
            else
    ...
    for _, component in pairs(components) do
        if type(component) == "table" then
            for _, subComponent in pairs(component) do
                if subComponent and typeof(subComponent) == "Instance" then
                    subComponent.Visible = false
                end
            end
        elseif component and typeof(component) == "Instance" then
...
function ESP:removeESP(player)
    if self.espCache[player] then
        self:hideComponents(self.espCache[player])
        ...
        for _, player in pairs(players:GetPlayers()) do
            if player ~= LocalPlayer then
                ...
                if character and character:FindFirstChild("Humanoid") and character:FindFirstChild("HumanoidRootPart") then
                    if not self.espCache[player] then
                        self.espCache[player] = self:createComponents()
                    end
                    self:updateComponents(self.espCache[player], character, player)
                elseif self.espCache[player] then
